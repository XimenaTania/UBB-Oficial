...
import {DeletePerson} from "../../dashboard-screen/modals/delete-person";

const MemberList = ({ data , item, setItem}) => {
    const [modalShowIglesia, setModalShowIglesia] = React.useState(false);
    const [modalDeletePerson, setModalDeletePerson] = React.useState(false);
    ...
    return (
        <div className="three-column-grid">
                            ...
                            <label className='dateMember'>{convertTimestamp(itemS.dataPerson.date)}</label>*/}
                                ...
                                <Dropdown>
                                    ...
                                    <Dropdown.Menu className='dMenu'>
                                        ...
                                        <Dropdown.Item href="#/action-3" onClick={() => setModalDeletePerson(true)}><div className='dropdownContent'><label className='dropdownContentTitle'>Dar de baja</label> <img src={imgDelete} alt='imgHistory' width='15' height='20' /></div></Dropdown.Item>
                                    </Dropdown.Menu>
                                </Dropdown>
            ...
            />
            <DeletePerson
                show={modalDeletePerson}
                onHide={() => setModalDeletePerson(false)}
            />
        </div>
    );
};
